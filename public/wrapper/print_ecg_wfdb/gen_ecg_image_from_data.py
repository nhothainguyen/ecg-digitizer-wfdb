...
from public.wrapper.print_ecg_wfdb.helper_functions import find_files
from public.wrapper.print_ecg_wfdb.extract_leads import get_paper_ecg
from public.wrapper.print_ecg_wfdb.ImageAugmentation.augment import get_augment
from public.wrapper.print_ecg_wfdb.HandwrittenText.generate import get_handwritten
from public.wrapper.print_ecg_wfdb.CreasesWrinkles.creases import get_creased

import warnings
from .helper_functions import read_config_file
#read_config_file: Đọc file cấu hình định dạng YAML và trả về dưới dạng dictionary
# os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2' 
# warnings.filterwarnings("ignore")

os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"           # Tắt INFO và WARNING
os.environ["TF_ENABLE_ONEDNN_OPTS"] = "0"          # Tắt tăng tốc oneDNN (nếu muốn kết quả ổn định hơn)

# =================================================
# Resolve config.yaml (PACKAGE RESOURCE)
# =================================================
#PACKAGE_DIR = Path(__file__).resolve().parent
PACKAGE_DIR ="public/wrapper/print_ecg_wfdb"
def resolve_config_path(config_arg: str | None):
    if config_arg:
        cfg = Path(config_arg)

        # nếu là path tương đối → ưu tiên theo PACKAGE_DIR
        if not cfg.is_absolute():
            pkg_cfg = PACKAGE_DIR / cfg
            if pkg_cfg.is_file():
                return pkg_cfg.resolve()

            # fallback: theo cwd
            cwd_cfg = Path.cwd() / cfg
            if cwd_cfg.is_file():
    ...
    # default config trong package
    default_cfg = PACKAGE_DIR / "config.yaml"
    if not default_cfg.is_file():
        raise FileNotFoundError(
            f"Default config.yaml not found inside package: {default_cfg}"
        )
    return default_cfg
...
def get_parser():
    ...
    parser.add_argument('--config_file', type=str, default='public/wrapper/print_ecg_wfdb/config.yaml')
...
def run_single_file(args):
        ...
        font = str(FONTS_DIR / random.choice(os.listdir(FONTS_DIR)))
        ...
        config_path = resolve_config_path(args.config_file)
        configs = read_config_file(str(config_path))

...
from public.wrapper.print_ecg_wfdb.helper_functions import read_leads, convert_bounding_boxes_to_dict, rotate_bounding_box, get_lead_pixel_coordinate, rotate_points
...
def get_augment(input_file,output_directory,rotate=25,noise=25,crop=0.01,temperature=6500,bbox=False, store_text_bounding_box=False, json_dict=None):
    ...
    if json_dict is None or 'leads' not in json_dict:
        # Không có metadata → bỏ qua bbox-based augment
        lead_bbs = leadNames_bbs = lead_bbs_labels = []
        startTime_bbs = endTime_bbs = plotted_pixels = None
    else:
        lead_bbs, leadNames_bbs, lead_bbs_labels, startTime_bbs, endTime_bbs, plotted_pixels = \

...
############################################# Functions to draw information ######################################################
    ...
    def add_ID_data(self, filename="", age="NaN", sex="NaN"):
        col_left = (
            '%s' % (filename,),
            'ID: %s' % ("X"),
            '%s ans, %s' % ("NaN","NaN")
    ...
    def add_info_data(self, date = "NaN"):
    ...
    def add_mid_data(self,frq_vent = "NaN", int_pr= "NaN", QRS= "NaN", QT_QTc= "NaN", Axes= "NaN", Moy_RR= "NaN", QTcB= "NaN", QTcF= "NaN"):
        """ Print file and case info """
        col_left = (
            '' ,
            '%s bpm' % (frq_vent,),
    ...
    def add_plot_filter_text(self, freq="500"):
...
def Write_PDF (ecg, path_output, type_of_pdf, lead_IIc = ""):
    ...
    # Sauvegarde en PNG
    if path_output.endswith('.png'):
        renderPM.drawToFile(plot.draw, path_output, fmt="PNG")

    # Sauvegarde en PDF
    if path_output.endswith('.pdf'):

...
def get_creased(im_array,output_file,ifWrinkles=False, modification_file = "1.jpg", ifCreases=False,crease_angle=0,num_creases_vertically=3,num_creases_horizontally=2,bbox=False):
    ...
    # read wrinkle image as grayscale and convert to float in range 0 to 1
        wrinkle_file_name = os.path.join('wrinkles-dataset' , modification_file)
        #print(wrinkle_file_name)
    ...
    img = im_array.astype("float32") / 255.0
    hh, ww = img.shape[:2]
